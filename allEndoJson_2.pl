...
my $dbresref=&getdbresref($conncfg{"dbname"},
  ...
  where exists(select 1 from dc_json_endo_list b where a.businesscode=b.riskcode)");

my $loopnum=200;#单线程批量单数
my $maxproc=6;#最大线程数

sub createjson{
	my ($productid,$loopid)=@_;
	system("perl /ccicall/dc/script/endojson_bak_test_4.pl $productid $loopid");
	my $dbh=DBI->connect("DBI:Oracle:pccictst9","dc_admin","dc_admin");
	$dbh->do("drop table dc_json_driver_endo_$loopid");
}
...
foreach my $row (@{$dbresref->{"result"}}){
	...
	my $sth=$dbh->prepare("select count(1) from dc_admin.dc_json_endo_list a where a.riskcode='".$$row[0]."'");
	print DBI->errstr if DBI->err;
	$sth->execute();
	my $dbrowcount=$sth->fetchrow_array();
	$sth->finish();
	$dbh->disconnect();
	print "product_code:".$$row[0].",dbrowcount:".$dbrowcount."\n";
	...
	for(my $count=0;$count<$dbrowcount;$count+=$loopnum){
		...
		$sth=$dbh->prepare("create table dc_json_driver_endo_$loopid as select policyno driver_key,1 lv from dc_json_endo_list where rownum<=$loopnum and riskcode='$$row[0]'");
		$sth->execute;
		$sth->finish;
		$dbh->do("delete from dc_json_endo_list a where exists(select 1 from dc_json_driver_endo_$loopid b where a.policyno=b.driver_key and b.lv=1)");
		$dbh->disconnect;
