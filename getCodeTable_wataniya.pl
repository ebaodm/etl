...
our $dbh=DBI->connect("DBI:Oracle:o46g4","wataniya_src_2","wataniya_src_2");
...
while(my $codetbid=$sth->fetchrow_array){
	...
	$res=$ua->request($req);
	print 'get err ,url=http://'.$server.':8080/dd/public/codetable/v1/data/list/'.$codetbid.'?access_token='.$token and next if ! $res->is_success;
	my $json=new JSON;
