...
$res=$ua->request($req);
die 'get http://'.$server.':8080/product/prd/v1/query/getAllProductList error' if ! $res->is_success;
my $productjson=decode_json($res->content) or die "非法JSON";
our $dbh=DBI->connect("DBI:Oracle:ccic_dev","dc_admin",'ccic726!') or die "连接数据库失败：".DBI->errstr;
foreach my $row (@{$productjson}){
	...
	$req=HTTP::Request->new(GET=>'http://'.$server.':8080/dd/public/dictionary/v1/resource/schema/full?modelName=Policy&objectCode=Policy&contextType=-2&referenceId='.$productid.'&access_token='.$token);
	$res=$ua->request($req);
	print 'get err ,url=http://'.$server.':8080/dd/public/dictionary/v1/resource/schema/full?modelName=Policy&objectCode=Policy&contextType=-2&referenceId='.$productid.'&access_token='.$token and next if ! $res->is_success;
	my $json=new JSON;
	...
	if(defined $dbmd5str){
		if($md5str eq $dbmd5str){
			...
			main($jsobj);
			$dbh->do("update PRODUCT_MD5_HIS set md5str='$md5str' where productid='$productid'") or (print DBI->errstr and next);
		}
	}else{
		main($jsobj);
		$dbh->do("insert into PRODUCT_MD5_HIS(productid,md5str) values('$productid','$md5str')") or (print DBI->errstr and next);
	}
}
$dbh->disconnect;
