...
my ($tbname,$apiserver,$user,$pwd,$dbname,$dbuser,$dbpwd,$maxrows,$maxproc,$etlid)=("T_PA_PL_PERSON_INSURED_L","10.1.14.178","20001",'eBao1234',
	"pccictst9","dc_test","dc_test",4999,10,"groupListEs_Endo.pl");
...
$sql=~s/\,$//;
$sql.=",b.endo_id AS ENDOID from ".$tbname." a,dm_pa_eds_endorsement b where a.endo_no=b.endo_no";
...
while(my $row=$sth->fetchrow_hashref){
	${$alldata}[$count]->{"Schema"}="InsuredEndo";
	foreach my $key (keys%{$row}){
		next if ! defined $row->{$key};
		${$alldata}[$count]->{"ParentId"}=$row->{"ENDOID"} and next if $key eq "ENDOID";
