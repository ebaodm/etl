...
sub getmain{
	...
	$req=HTTP::Request->new(GET=>'http://'.$server.':8080/dd/public/dictionary/mgmt/v1/generateFullUiResourceSchemaWithoutField?modelName=Policy&objectCode=POLICY&contextType=-2&referenceId='.$_[0].'&access_token='.$token);
	$res=$ua->request($req);
	print encode("gbk",decode("utf8",$res->content())) and die 'get http://'.$server.':8080/dd/public/dictionary/mgmt/v1/generateFullUiResourceSchemaWithoutField?modelName=Policy&objectCode=POLICY&contextType=-2&referenceId='.$_[0].'&access_token='.$token if ! $res->is_success;
	my $json=new JSON;
	...
	our $dbh=DBI->connect("DBI:Oracle:".$tnsname,$dbuser,$dbpwd) or die "连接数据库失败：".DBI->errstr;
...
$sth=$dbh->prepare("select distinct a.productid from product_product a ,dc_valid_productid b where a.productid=b.productid") or (print DBI->errstr and $dbh->disconnect and exit);
$sth->execute;
my $productref=$sth->fetchall_arrayref;
$sth->finish;
$dbh->disconnect;
my ($procnum,$collectnum,$count)=(0,0,0);
$SIG{CHLD}=sub{$procnum--};
foreach my $loopid (@{$productref}){
	my $pid=fork;
	if(! defined $pid){
		print "fork err!\n";
	}
