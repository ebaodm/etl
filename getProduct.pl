...
$dbh->disconnect;
$dbh=DBI->connect("DBI:Oracle:$tnsname",$dbuser,$dbpwd,{AutoCommit=>0}) or die "连接数据库失败：".DBI->errstr;
$sth=$dbh->prepare("select distinct productid from product_product") or (print DBI->errstr and $dbh->disconnect and exit);
$sth->execute;
my $productref=$sth->fetchall_arrayref;
$sth->finish;
$dbh->disconnect;
unlink "/home/oracle/dc/ccic/script/log_getProductObj.log" if -f "/home/oracle/dc/ccic/script/log_getProductObj.log";
foreach my $loopid (@{$productref}){
	system("perl /home/oracle/dc/ccic/script/getProductObj.pl $$loopid[0]  >> /home/oracle/dc/ccic/script/log_getProductObj.log");
}
$dbh=DBI->connect("DBI:Oracle:$tnsname",$dbuser,$dbpwd,{AutoCommit=>0}) or die "连接数据库失败：".DBI->errstr;
...
$sth->finish;
$dbh->disconnect;
unlink "/home/oracle/dc/ccic/script/log_getObjAttr.log" if -f "/home/oracle/dc/ccic/script/log_getObjAttr.log";
foreach my $objid (@{$objref}){
	system ("perl /home/oracle/dc/ccic/script/getObjAttr.pl $$objid[0] >> /home/oracle/dc/ccic/script/log_getObjAttr.log");
}
